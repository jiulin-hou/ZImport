...
( cd "$APP_DIR" && PYTHONPATH="$APP_DIR" "$APP_DIR/venv/bin/python" \
  -c "import zimport.web, zimport.worker, zimport.zimbra_folders, zimport.zimbra_search; print('  模块导入 OK')" )
...
下一步:
  ...
  1. 安装并启动服务:
       cp $APP_DIR/deploy/zimport-web.service $APP_DIR/deploy/zimport-worker.service \\
          /etc/systemd/system/
