...
# --- 写版本号 -----------------------------------------------------------
log "写入版本号 $VERSION → zimport/__init__.py"
printf '__version__ = "%s"\n' "$VERSION" > zimport/__init__.py

# --- 提交 + 打标签 ------------------------------------------------------
log "提交并打标签 $TAG"
git add zimport/__init__.py CHANGELOG.md
...
log "生成交付包 $ARCHIVE"
git archive --format=tar.gz --prefix="zimport/" -o "$ARCHIVE" "$TAG"
